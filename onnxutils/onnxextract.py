...
def extract_compute_graph_taxonomy_style(filename,savePath='./outdata',model_name='',parameters=False,writeFile=True):
    ...
    for node in graph.node:
        ...
        #    if inp not in existing_names:
        #        placeholder = {
        #            "op": "input",
        #            "name": inp,
        #            "target": node.name,
        #            "parameters": {}  # You can include type/shape info if desired
        #        }
...
class ONNXProgram:
    ...
    def inference_extraction(self,filename,model_type,model_name,savePath='./outdata'):
        ...
        #    print(key)
        #    print(ort_outs[key])
        #    print(ort_outs[key].shape)
        #    print(len(ort_outs[key].shape))
        #    print(input())
        # model = onnx.load(filename)
        # graph = model.graph
        # print(graph.output)
        # input()
        # all_node_names = [output for node in graph.node for output in node.output ]
        ...
        #    try:
        #        outputs = session.run([i], input_data_dict)
        #        print('yeh',i,outputs[0])
        #    except:
        #        print('nope: ', i)
